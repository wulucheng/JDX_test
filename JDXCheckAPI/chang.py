...
def send_requests(method, url, header=1, data=None, token=None, verify = False):
    ...
    if method == "GET":
        ...
        resp = requests.get(full_url, data, headers = full_header, verify = verify)
    else:
...
if __name__ == '__main__':
    case = {'param': 36047, 'name': '人工询价', 'type': '质检入参', 'url': '/app/inspection/report/create/report/default'}
    inquiry_url = "jdx-qa-service/" + case["url"]
    param_data = {"productId": case["param"]}
    token = '92a67d8b95ab051710762681cda29bae'
    resp = send_requests(method="get", url=inquiry_url, header=1, data = param_data, token=token)
    print(case["param"])
